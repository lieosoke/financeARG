...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Session token is unencrypted and insecure').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: User authentication sessions are not encrypted, tokens are insecure, or do not expire correctly requiring re-login as per the test plan.')
