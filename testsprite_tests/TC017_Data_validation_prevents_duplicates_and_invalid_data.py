...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Duplicate entry detected for jamaah ID or package code').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Backend and frontend validations did not prevent duplicate entries or show the expected validation message.')
