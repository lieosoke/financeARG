...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Seat Overbooking Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The system did not prevent seat overbooking as expected. The package seat availability limits were not enforced, allowing jamaah to be assigned beyond capacity.')
