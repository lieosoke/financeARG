...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=No matching records found for search query').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Global search functionality did not return correct results for jamaah, packages, vendors, and transactions as expected.')
